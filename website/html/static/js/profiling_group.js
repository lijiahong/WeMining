	  ...
	  var link = svg_1.selectAll(".link")
		...
		.enter().append("line")
		  ...
		  .attr("class", function(d) { 
								  if(d.value > 5){return "class link_level2";}
								  else{ return "class link_level1" }})
		  .style("stroke", function(d) { 
								  if(d.value > 5){return "#fff" ;}
								  else{ return "#eee"; }});
...
function display_rgraph_vis(){
	
	
	var sex = new RGraph.Pie('sexCanvas',[1500,2500]);
	sex.Set('chart.title', '性别分布');
	sex.Set('chart.labels',['女性','男性']);
		sex.Set('chart.tooltips.effect','expand');
	sex.Set('chart.tooltips.event','onmousemove');
		sex.Set('chart.tooltips', ['女性','男性']);
	sex.Set('chart.highlight.style','3d');
	sex.Set('chart.colors', ['#2873ac','#FE9A2E']);//['#81BEF7','#F7819F']);
	sex.Set('chart.linewidth',2);
	sex.Set('chart.strokestyle','black');
	sex.Set('chart.tooltips.effect','fade');
	sex.Draw();

	var verified = new RGraph.Pie('verifiedCanvas',[1000,3000]);
	verified.Set('chart.labels',['认证','普通']);
		verified.Set('chart.tooltips.effect','expand');
	verified.Set('chart.tooltips.event','onmousemove');
		verified.Set('chart.tooltips', ['认证','普通']);
	verified.Set('chart.highlight.style','3d');
	verified.Set('chart.colors', ['#2873ac','#FE9A2E']);//['#81BEF7','#F7819F']);
	...
	var age = new RGraph.Bar('ageCanvas',[200,800,1000,600,400]);
	age.Set('chart.labels',['10以下','11-20','21-30','31-40','40以上']);
	age.Set('chart.labels.above',true);
	age.Set('chart.background.grid', false);
	age.Set('chart.background.grid.vlines', false);
	age.Set('chart.background.grid.hlines', false);
	age.Set('chart.tooltips.effect','expand');
	age.Set('chart.tooltips.event','onmousemove');
		age.Set('chart.tooltips', ['10以下','11-20','21-30','31-40','40以上']);
	age.Set('chart.highlight.style','3d');	    
		age.Set('chart.strokestyle', 'black');
	age.Set('chart.colors',['#2873ac','#2873ac','#2873ac','#2873ac','#2873ac']);
		age.Set('chart.width',450);
	age.Set('chart.height',150);
		age.Set('gutter.left', 50);
		age.Set('gutter.right', 5);
	age.Draw();
	 
	/*
	var working = new RGraph.Bar('workingCanvas',[2500,1600,1700,1800,1900,2500,1600,1700,1800,1900,2400,1300,1300,2400,1500]);
	working.Set('chart.labels',['IT','金融','证券','机械','电子','IT','金融','证券','机械','电子','IT','金融','证券','机械','电子']);	      
		working.Set('chart.strokestyle', 'black');
	working.Set('labels.above',true);
	working.Set('chart.tooltips.effect','expand');
	working.Set('chart.tooltips.event','onmousemove');
		working.Set('chart.tooltips', ['IT','金融','证券','机械','电子','IT','金融','证券','机械','电子','IT','金融','证券','机械','电子']);
	working.Set('chart.highlight.style','3d');
	working.Set('chart.colors',['#FA5858','#FA5858','#FA5858','#FA5858','#FA5858','#FA5858','#FA5858','#FA5858','#FA5858','#FA5858','#FA5858','#FA5858','#FA5858','#FA5858','#FA5858']);
	working.Set('chart.width',800);
	working.Set('chart.height',350);
		working.Set('gutter.left', 120);
		working.Set('gutter.right', 80);
	working.Draw();*/
}
...
function drawStaticMap() {
    ...
    var data = {"count": {"\u9752\u6d77": [1, 22, "0.1%"], "\u6cb3\u5357": [6, 10, "1.1%"], "\u5c71\u4e1c": [4, 14, "0.7%"], "\u56db\u5ddd": [22, 6, "4.3%"], "\u6c5f\u82cf": [6, 11, "1.1%"], "\u798f\u5efa": [3, 17, "0.5%"], "\u6d59\u6c5f": [35, 3, "6.8%"], "\u6e56\u5317": [3, 18, "0.5%"], "\u5929\u6d25": [9, 9, "1.7%"], "\u6c5f\u897f": [6, 12, "1.1%"], "\u5e7f\u4e1c": [72, 2, "14.0%"], "\u9999\u6e2f": [3, 19, "0.5%"], "\u5317\u4eac": [232, 1, "45.4%"], "\u6d77\u5916": [20, 7, "3.9%"], "\u5176\u4ed6": [28, 5, "5.4%"], "\u6cb3\u5317": [2, 21, "0.3%"], "\u91cd\u5e86": [6, 13, "1.1%"], "\u6e56\u5357": [13, 8, "2.5%"], "\u5b89\u5fbd": [4, 15, "0.7%"], "\u4e91\u5357": [2, 20, "0.3%"], "\u4e0a\u6d77": [30, 4, "5.8%"], "\u6d77\u5357": [4, 16, "0.7%"]}, "color": {"\u9752\u6d77": "#2873AC", "\u6cb3\u5357": "#FFD914", "\u5c71\u4e1c": "#51B133", "\u56db\u5ddd": "#FD8D24", "\u6c5f\u82cf": "#FFD914", "\u798f\u5efa": "#29AC80", "\u6d59\u6c5f": "#FF5B25", "\u6e56\u5317": "#29AC80", "\u5929\u6d25": "#FFD914", "\u6c5f\u897f": "#FFD914", "\u5e7f\u4e1c": "#FF5B25", "\u9999\u6e2f": "#29AC80", "\u5317\u4eac": "#FF5B25", "\u6d77\u5916": "#FD8D24", "\u5176\u4ed6": "#FD8D24", "\u6cb3\u5317": "#2873AC", "\u91cd\u5e86": "#51B133", "\u6e56\u5357": "#FD8D24", "\u5b89\u5fbd": "#51B133", "\u4e91\u5357": "#29AC80", "\u4e0a\u6d77": "#FF5B25", "\u6d77\u5357": "#51B133"}, "summary": [[1, "\u5317\u4eac", "45.4%"], [2, "\u5e7f\u4e1c", "14.0%"], [3, "\u6d59\u6c5f", "6.8%"], [4, "\u4e0a\u6d77", "5.8%"], [5, "\u5176\u4ed6", "5.4%"], [6, "\u56db\u5ddd", "4.3%"], [7, "\u6d77\u5916", "3.9%"], [8, "\u6e56\u5357", "2.5%"], [9, "\u5929\u6d25", "1.7%"], [10, "\u6cb3\u5357", "1.1%"]]};

    var city_color = data['color'];
    var city_count = data['count'];
    var city_summary = data['summary'];
    table_str = '<table align="center">'
    row_str = '';
    for(var row=0;row<city_summary.length;row++){
	row_str += '<tr>';
	col_str = '';
	row_cell = city_summary[row];
	for(var col=0;col<row_cell.length;col++){
	    col_str += '<td>'+row_cell[col]+'</td>';
	}
	row_str += col_str
	row_str += '</tr>';
    };
    table_str += row_str;
    table_str += '</table>'
    summary.html('用户地域分布:</br>'+table_str);
    for (var state in china) {
	//分省区域着色
	china[state]['path'].color = Raphael.getColor(0.9);
	(function (st, state) {
	    city = china[state]['name'];
		...
		if(e.type == 'mouseenter'){
		    city = china[state]['name'];
		    count = city_count[city][0];
		    rank = city_count[city][1];
		    percent = city_count[city][2];
		    tiplayer.html(city+'</br>数量:'+count+'</br>排名:'+rank+'</br>百分比:'+percent).css({'opacity':'0.75','top':(e.pageY+10)+'px','left':(e.pageX+10)+'px'}).fadeIn('normal');
		}else{
